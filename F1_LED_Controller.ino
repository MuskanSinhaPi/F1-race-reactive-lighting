...
int pendingJolpicaWDC = 0;
int pendingJolpicaWDCPolls = 0;
...
void fetchNextRaceFromJolpica() {
  ...
  // Start immediately after the last completed race.
  int startRound = EEPROM.read(EEPROM_LAST_RACE_ROUND) + 1;
  ...
  const int MAX_ATTEMPTS_PER_ROUND = 3;
  ...
  for (int round = startRound; round <= totalRounds; round++) {
    ...
    for (int attempt = 1;
      ...
      Serial.printf(
        "Jolpica next: trying round %d (attempt %d/%d)\n",
        round, attempt, MAX_ATTEMPTS_PER_ROUND
      );

      if (!https.begin(client, url)) {
        Serial.printf(
          "Jolpica round %d: begin() FAILED\n",
          round
        );
        delay(500);
        continue;
      ...
      Serial.printf(
        "Jolpica round %d HTTP: %d\n",
        round, code
      );

      if (code != HTTP_CODE_OK) {
        Serial.printf(
          "Jolpica round %d failed — retrying same round\n",
          round
        );
        https.end();
        delay(500);
      ...
      if (!name || !date || !rtime || !rndS) {
        Serial.printf(
          "Jolpica round %d: incomplete race data\n",
          round
        );
        delay(500);
        continue;
      ...
      Serial.printf(
        "Round %s: %s %s %s\n",
        rndS, name, date, rtime
      );

      // Successfully retrieved and parsed this round.
      roundSucceeded = true;

      // If this race has already happened, move to the next round.
      if (raceEpoch <= now) {
        Serial.printf(
          "Round %d already passed — trying next round\n",
          round
        );
        break;
      }
    ...
    // ============================================================
    // IMPORTANT:
    // If this round could not be retrieved after all attempts,
    // DO NOT skip it and try the next round.
    // ============================================================

    if (!roundSucceeded) {
      Serial.printf(
        "Jolpica round %d: all %d attempts failed — "
        "ABORTING next-race search\n",
        round,
        MAX_ATTEMPTS_PER_ROUND
      );

      Serial.println(
        "Keeping previous race schedule."
      );
...
void checkWDCViaJolpica() {
  ...
  if (code != 200) {
    https.end();
    Serial.printf("WDC check: HTTP %d\n", code);
    return;
  }

  JsonDocument filter;

  filter["MRData"]["StandingsTable"]["StandingsLists"][0]
        ["DriverStandings"][0]["Constructors"][0]["name"] = true;
  ...
  if (err) {
    Serial.printf("WDC check: parse err: %s\n", err.c_str());
    return;
  }
  ...
  if (!con) {
    Serial.println("WDC check: constructor missing");
    return;
  }
  ...
  if (cid < 1 || cid > 11) {
    Serial.println("WDC check: unknown constructor");
    return;
  }

  Serial.printf(
    "WDC check: standings candidate = %s\n",
    teamNames[cid]
  );

  // First observation of a candidate.
  if (pendingJolpicaWDC != cid) {
    pendingJolpicaWDC = cid;
    pendingJolpicaWDCPolls = 1;

    Serial.printf(
      "WDC check: candidate recorded (%s), waiting for confirmation poll\n",
      teamNames[cid]
    );
  ...
  // Same candidate seen again on a later poll.
  pendingJolpicaWDCPolls++;

  Serial.printf(
    "WDC check: %s seen %d consecutive polls\n",
    teamNames[cid],
    pendingJolpicaWDCPolls
  );

  if (pendingJolpicaWDCPolls >= 2) {
    Serial.printf(
      "Auto-check: WDC confirmed via Jolpica standings: %s\n",
      teamNames[cid]
    );
